...
class AccountEsTestCase(CompanyTestMixin, ModuleTestCase):
    ...
    @with_transaction()
    def test_supplier_tax_deductible_rate_zero_with_account_category(self):
        pool = Pool()
        FiscalYear = pool.get('account.fiscalyear')
        Account = pool.get('account.account')
        Party = pool.get('party.party')
        ...
        with set_company(company):
            ...
            unit, = ProductUom.search([
                ('name', '=', 'Unit'),
            ], limit=1)
            _, tax = get_tax('iva_sop_21')

            category = ProductCategory(
                name='Non Deductible Purchase Category')
            category.accounting = True
            category.account_expense = expense
            category.supplier_taxes = [tax]
            category.supplier_taxes_deductible_rate = Decimal('0')
            category.save()

            template, = ProductTemplate.create([{
                'name': 'Non Deductible Purchase Product',
                'default_uom': unit.id,
                'type': 'service',
            ...
            with Transaction().set_context(
                ...
                self.assertEqual(line.taxes_deductible_rate, Decimal('0'))
                self.assertEqual({t.id for t in line.taxes}, {tax.id})
                self.assertEqual(amount, Decimal('121.00'))
                ...
                self.assertEqual(invoice.untaxed_amount, Decimal('121.00'))
